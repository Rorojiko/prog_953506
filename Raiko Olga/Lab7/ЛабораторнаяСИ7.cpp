/* Приорбанк. Сведения о вкладчиках банка: номер лицевого счета,
категория вклада, паспортные данные, текущая сумма вклада
(остаток). Операции приема и выдачи сумм по вкладу (разрешенные
категорией вклада), расчет процентов на заданную дату. Переводы
денежных средств между клиентами, подготовка квитанций.
Отчеты: выписка счета (все операции, проведенные с данным
счетом в текущем году); список всех клиентов банка и поиск по
нему; сортировка по остаткам вкладов клиентов; сумма остатков.*/
...
void show_menu(void)//функция для вывода меню
{
	printf("Menu\n");
...
void add_account(struct List* list)//функция ввода данных
{
...
int main(void)
{
	...
	while (1)
	{
		...
		switch (switch_menu)// проверка и отчистка списка
		{
		case 1:
			add_account(list);
			break;
